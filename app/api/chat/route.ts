...
export const runtime = "edge"  // 建議 edge，反應快

export async function POST(req: NextRequest) {
  try {
    // 1) 免費配額檢查（Pro/Pro+ 直接通過；Free 每日 n 次）
    ...
    if (!apiKey) return NextResponse.json({ error: "未設定 GROQ_API_KEY" }, { status: 500 })
    ...
    const r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "你是專精於家族傳承、保單結構、台美跨境稅務的助理。請用條列、標題與表格呈現重點，最後給出行動清單。"
          },
          { role: "user", content }
        ],
        temperature: 0.3
