  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-6">
      <div className="w-full max-w-md border rounded-2xl p-6 bg-white shadow-sm">
        ...
        <p className="text-sm text-gray-600 mb-6">請輸入邀請碼以存取顧問工具（/tools/*）。</p>
        <label className="flex flex-col gap-1 mb-4">
          <span>邀請碼</span>
          <input value={code} onChange={(e) => setCode(e.target.value)}
                 className="rounded-lg border p-2" placeholder="請輸入邀請碼" />
        </label>
        ...
        <p className="text-xs text-gray-500 mt-4">登入成功後，系統會在瀏覽器存放授權 Cookie（僅用於放行顧問頁面）。</p>
      </div>
    </div>
  );
}
