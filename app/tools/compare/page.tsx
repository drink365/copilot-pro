...
export default function CompareTool() {
  ...
  const block = (title: string, data?: EstateRes & { totalGiftFree?: number }) => (
    <div className="rounded-xl border p-4 space-y-1">
      ...
      {data ? (
        <>
          {"totalGiftFree" in data && (
            <div>
              累計免稅贈與：<b>{(data as any).totalGiftFree?.toLocaleString() ?? 0}</b>
            </div>
          )}
          <div>課稅遺產：<b>{data.taxableEstate.toLocaleString()}</b></div>
          <div>級距：<b>{data.bracket}%</b></div>
          <div className="text-lg">
            預估遺產稅：<b className="text-emerald-700">{data.tax.toLocaleString()} 元</b>
          </div>
        </>
      ) : (
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 space-y-6">
      <CalculatorCard
        title="📊 現況 vs. 逐年贈與 vs. 組合方案（贈與＋保險＋信託）"
        footer={
          <button
            onClick={run}
            disabled={loading}
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <div className="text-sm text-gray-500">
                說明：組合方案以「較積極贈與＋保險預留稅源／信託控管」為示意，用於會談引導。
              </div>
            </div>
          </div>
        )}
