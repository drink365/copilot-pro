  ...
  const run = async () => {
    ...
    const r = await fetch("/api/tax/estate", {
      ...
      body: JSON.stringify({
        ...
        numChildren,
        includeSpouse,
        includeFuneralDeduction: includeFuneral,
      }),
    });
    setRes(await r.json());
    setLoading(false);
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={includeFuneral}
              onChange={(e) => setIncludeFuneral(e.target.checked)}
            />
        ...
        {res && (
          <div className="rounded-xl bg-gray-50 p-4 space-y-2">
            <div>
              扣除合計：
              <b>{res.deductions.total.toLocaleString()}</b>
              （基本 {res.deductions.base.toLocaleString()}＋配偶 {res.deductions.spouse.toLocaleString()}＋子女 {res.deductions.children.toLocaleString()}＋喪葬 {res.deductions.funeral.toLocaleString()}）
            </div>
